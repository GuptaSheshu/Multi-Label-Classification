...
from model_loader1_dnn2 import get_model
...
pts=50
Ntrains=np.linspace(10,ntrain,pts)
# Ntrains=[5718, 5922, 6126, 6330, 6534, 6737,
# 6941, 7145, 7349, 7553, 7757, 7961, 
# 8165, 8368, 8572, 8776, 8980, 9184,
# 9388, 9592, 9796, 10000]
# Ntrains=[7757, 7961, 
# 8165, 8368, 8572, 8776, 8980, 9184,
# 9388, 9592, 9796, 10000]
...
lr_folder = os.listdir('./Data1Models/')

dnn_model = 'dnn2'
# Dnn1 - inp -> 6 -> out
# Dnn2 - inp -> 32 -> 8 -> out
# Dnn3 - inp -> 64 -> 32 -> 16 -> out 
for lr in list_lr:
    if not ('lr_'+str(lr) in lr_folder):
        os.mkdir('./Data1Models/lr_'+str(lr))
    # if lr==1e-4 or lr==1e-3:
    #     epochs=150
    # if lr==1e-2:
    #     wtdecay=1e-3
    ...
    for (n_idx, n) in enumerate(Ntrains):
        ...
        optimizer = torch.optim.Adam(model_params, lr=lr, weight_decay=wtdecay)
        ...
        #        print('Half the learning rate{}'.format(n_iter))
            ...
            if epoch % 9 == 0 and n_idx==len(Ntrains)-1:
                ...
                torch.save(
                    state,'./Data1Models/lr_'+str(lr)+'/sener_'+dnn_model+
                    '-{}-{}-{}-{}-{:.4f}.pt'.format(
        ...
        #save the metrics
        np.save("./../Datasets/Data1/Metrics/sener_"+dnn_model+"_lr_"+str(lr)+
                "_metrics_data1.npy",np.array(sener_metrics))
        dictionary={'sener': sener_metrics}
        savemat("./../Datasets/Data1/Metrics/sener_"+dnn_model+"_lr_"+str(lr)+
            "_metrics_data1.mat",
            dictionary
            )
        ...
        df.to_excel("./../Datasets/Data1/Metrics/sener_"+dnn_model+"_lr_"+str(lr)+
