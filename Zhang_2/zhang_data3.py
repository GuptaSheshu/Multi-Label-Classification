...
#Our Modules
from model_loader import fbeta_dnn3
...
train_Data_file = './../Datasets/Data3/Xtrain_2021-12-04.npy'
val_Data_file = './../Datasets/Data3/Xval_2021-12-04.npy'
test_Data_file = './../Datasets/Data3/Xtest_2021-12-04.npy'
train_binlabel_file = './../Datasets/Data3/Ytrainbin_2021-12-04.npy'
val_binlabel_file = './../Datasets/Data3/Yvalbin_2021-12-04.npy'
test_binlabel_file = './../Datasets/Data3/Ytestbin_2021-12-04.npy'
...
Ntrains=np.linspace(3726,3726,1)
...
##################################################
##################################################
list_lr = np.logspace(-3,-1,3)
for lr in list_lr:
    if not ('lr_'+str(lr) in lr_folder):
        os.mkdir('./Data3Models/lr_'+str(lr))
    ...
    for (n_idx, n) in enumerate(Ntrains):
        ...
        Yv = torch.tensor(np.float64(np.load(val_binlabel_file)))
        ...
        optmizer= optim.Adam(net.parameters(), lr= lr, weight_decay=1e-3)
        ...
        for e in range(epochs):
    	    ...
    	    # Save after every certain epochs
                torch.save(
                    net.state_dict(),'./Data3Models/lr_'+str(lr)+'/zhang-{}-{}-{}-{}-{:.4f}.pt'.format(
                        n,e+1,batchsize,lr,
                        validation_fbeta)
                    )
        ...
        #save the metrics
        np.save("./../Datasets/Data3/Metrics/zhang_lr_"+str(lr)+
                "_metrics_data3.npy",np.array(zhang_metrics))
        dictionary={'zhang': zhang_metrics}
        savemat("./../Datasets/Data3/Metrics/zhang_lr_"+str(lr)+
            "_metrics_data3.mat",
            dictionary
            )
        ######################### Evaluation Metrics#####################
    ...
    df.to_excel("./../Datasets/Data3/Metrics/zhang_lr_"+str(lr)+
            "_metrics_data3.xlsx" ,index=False)
    ##################################################
    ##################################################
    ##################################################
    ########################### Save Fbeta Accuracy ########################
    # ymdstr = time.strftime("%Y-%m-%d")
    # np.save("./../Datasets/Data2/Metrics/zhang_lr_"+str(lr)+"_fbetaacc_data2_"+ymdstr+".npy",np.array(zhang_metrics))