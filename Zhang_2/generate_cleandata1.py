...
########This code can be run separately from above code###########
...
ntest=3000
yymmdd="2021-11-13"
Ptest = np.load("./../Datasets/Data1/Ptest_"+yymmdd+".npy")
Ptest = Ptest[:ntest,:]
...
Ytestbin = np.load("./../Datasets/Data1/Ytestbin_"+yymmdd+".npy")
Ytestbin=Ytestbin[:ntest,:]
...
print(bayes_fbeta)
np.save("./../Datasets/Data1/Clean_Metrics/bayes_fbeta_cleandata1_"+
        ".npy",np.array([bayes_fbeta]))
dictionary={'bayes_fbeta': np.array([bayes_fbeta])}
savemat("./../Datasets/Data1/Clean_Metrics/bayes_fbeta_cleandata1.mat",
        dictionary)
columns=['Test-size', 'Fbeta-score']
temp=np.zeros((1,2))
temp[0,:]=np.array([ntest, bayes_fbeta])
df = pandas.DataFrame(temp, columns=columns)
df.to_excel("./../Datasets/Data1/Clean_Metrics/bayes_fbeta_cleandata1"+
            ".xlsx" ,index=False)
