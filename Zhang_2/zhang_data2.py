...
#Our Modules
from model_loader import fbeta_dnn2
...
for lr in list_lr:
    ...
    for (n_idx, n) in enumerate(Ntrains):
        ...
        for e in range(epochs):
    	    ...
    	    # Save after every certain epochs
                torch.save(
                    net.state_dict(),'./Data2Models/lr_'+str(lr)+'/zhang-{}-{}-{}-{}-{:.4f}.pt'.format(
                        n,e+1,batchsize,lr,
                        validation_fbeta)
                    )
        ...
        #save the metrics
        np.save("./../Datasets/Data2/Metrics/zhang_lr_"+str(lr)+
                "_metrics_data2.npy",np.array(zhang_metrics))
        dictionary={'zhang': zhang_metrics}
        savemat("./../Datasets/Data2/Metrics/zhang_lr_"+str(lr)+
            "_metrics_data2.mat",
            dictionary
            )
        ######################### Evaluation Metrics#####################
    ...
    df.to_excel("./../Datasets/Data2/Metrics/zhang_lr_"+str(lr)+
